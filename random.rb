sf_koders = ["Annalyce","Emily C","Julia L","Lizzy", "Jaia", "Ossema", "Sol", "Caroline", "Michelle", "Izzy", "Mai-Thanh", "Alicia", "Kiara", "Yongqi", "Jackie", "Steph", "Clarissa"].shuffle
...
# this sorts into five
def student_fives(array)
    until array.empty? == true
        puts "#{pretty(array.pop(1))}, #{pretty(array.pop(1))}, #{pretty(array.pop(1))}, #{pretty(array.pop(1))}, and #{pretty(array.pop(1))}"
        unless array.count % 5 == 0
            if array.count == 4
                puts "#{pretty(array.pop(1))}, #{pretty(array.pop(1))},#{pretty(array.pop(1))}, and #{pretty(array.pop(1))}"
            end
        end
